...
for co0 in co:
    ...
    for idx in np.arange(1, 21):
        ...
        try:
            ...
            folder = f"N{N}co{co0}T{T}/"
            inter = data_path + f"N{N}_cutoff{co0}_T{T}_inter_molecule_idx{idx}.npy" 
            intra = data_path + f"N{N}_cutoff{co0}_T{T}_intra_molecule_idx{idx}.npy" 
    ...
    out.append([co0, R1, R1_error, R1T[0], R1T_error[0], R1R[0], R1R_error[0], tau_T, tau_R, tau_T_error, tau_R_error])
...
np.savetxt("T1_TIP4P.dat", out, header="co R1 R1_error R1T R1T_error R1T R1R_error tauT tauR", fmt="%.8e")
