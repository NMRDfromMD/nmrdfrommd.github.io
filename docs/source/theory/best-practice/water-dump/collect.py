...
data_path = "/media/simon/data1/data-nmrdfrommd/TIP4P/analysis-varying-dump/results/"
...
for step in [1, 2, 4, 8, 16, 32, 64]:
    ...
    inter = data_path + f"N{N}_cutoff{co}_T{T}_inter_molecule_step{step}.npy" 
    intra = data_path + f"N{N}_cutoff{co}_T{T}_intra_molecule_step{step}.npy" 
    ...
    out.append([step, R1, R1_error, R1T[0], R1T_error[0], R1R[0], R1R_error[0], tau_T, tau_R, tau_T_error, tau_R_error])
    ...
    if step == 1:
        ...
        np.savetxt("GR_step1_TIP4P.dat", np.vstack([t, GR, GR_error]).T, header="t Gij Gij_error", fmt="%.8e")
        np.savetxt("GT_step1_TIP4P.dat", np.vstack([t, GT, GT_error]).T, header="t Gij Gij_error", fmt="%.8e")
    ...
    if step == 64:
        ...
        np.savetxt("GR_step64_TIP4P.dat", np.vstack([t, GR, GR_error]).T, header="t Gij Gij_error", fmt="%.8e")
        np.savetxt("GT_step64_TIP4P.dat", np.vstack([t, GT, GT_error]).T, header="t Gij Gij_error", fmt="%.8e")
